...
def run_exp(dataset_desc, perturbation_desc, model_desc):
    # Load data
    X, y = load_data(dataset_desc, scaling=True) # desactibamos el scaling para evaluar los resultados de iris. 
    n_dim = X.shape[1]
    ...
    for i in range(X.shape[0]):
        ...
        y_cf = X_perturbed_red[i,:] # array de datos proyectados "perturbados" (dos datos proyectados x, y)
        ...
        raw_results["X_perturbed"].append(X_perturbed[i])  # esto lo añadí yo. Son los datos originales perturbados
...
if __name__ == "__main__":
    ...
    dataset_desc = "breastcancer" #sys.argv[1]
    perturbation_desc = "none" #sys.argv[2]
    n_perturbed_features = 0 #int(sys.argv[3])
    ...
    for _ in range(n_iter):
        res = run_exp(dataset_desc, perturbation_desc, model_desc)
        results.append(res)
